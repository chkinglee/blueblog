...
type Repo interface {
	...
	Keys(pattern string, options ...Option) ([]string, error)
	Close() error
...
func (c *cacheRepo) HMGet(key string, fields []string, options ...Option) ([]interface{}, error) {
	ts := time.Now()
	opt := newOption()
	defer func() {
		if opt.Trace != nil {
			opt.Redis.Timestamp = time_parse.CSTLayoutString()
			opt.Redis.Handle = "HMGet"
			opt.Redis.Key = key + "----" + strings.Join(fields, ",")
			opt.Redis.CostSeconds = time.Since(ts).Seconds()
			opt.Trace.AppendRedis(opt.Redis)
		}
	}()
...
func (c *cacheRepo) HKeys(key string, options ...Option) ([]string, error) {
	ts := time.Now()
	opt := newOption()
	defer func() {
		if opt.Trace != nil {
			opt.Redis.Timestamp = time_parse.CSTLayoutString()
			opt.Redis.Handle = "hkeys"
			opt.Redis.Key = key
			opt.Redis.CostSeconds = time.Since(ts).Seconds()
			opt.Trace.AppendRedis(opt.Redis)
		}
	...
	value, err := c.client.HKeys(key).Result()
	if err != nil {
		return nil, errors.Wrapf(err, "redis get key: %s err", key)
	}
...
func (c *cacheRepo) Keys(pattern string, options ...Option) ([]string, error) {
	ts := time.Now()
	opt := newOption()
	defer func() {
		if opt.Trace != nil {
			opt.Redis.Timestamp = time_parse.CSTLayoutString()
			opt.Redis.Handle = "keys"
			opt.Redis.Key = pattern
			opt.Redis.CostSeconds = time.Since(ts).Seconds()
			opt.Trace.AppendRedis(opt.Redis)
		}
	}()
	...
	if err != nil {
		return nil, errors.Wrapf(err, "redis keys: %s err", pattern)
	}
